 ...
 * 7/13/2007 bcabana Removed all but 2 of 2d charts from sitraka
 * 8/20/2007 bcabana Axis Label Rotation Support per QM-6517, QM-6567 and QM-6539
 * 8/24/2007 bcabana QM-6657 Better label text measurement for rotated labels
 * 9/4/2007 bcabana Fixed notification inside of paint call
 * 1/21/2008 bcabana Better measurement of spacing
 * 2/26/2008 bcabana QM-7481 and QM-7523, QM-7677 Multi-y-axis fixes
 * 7/8/2008 bcabana QM-8094: Additional model visualization / interaction gadgets 
 * 
 */
...
public abstract class ValueAxis extends Axis
	...
	protected boolean useRotatedTickLabels = true;

	/** 
     * A structure defining the category label positions for each axis 
     * location. 
     * @since 8/20/07 by Brian Cabana to support rotation of value axis labels
     */
    private CategoryLabelPositions categoryLabelPositions;
   
    /** origin is calculated automatically based on lower bound */
    private boolean autoOrigin = true;
    
    /** The numerical origin value for the axis. */
    private double origin;

    /** A per axis setting to determine if the axis should draw the gridlines */
   ...
   /**
    ...
    protected ValueAxis(String label, TickUnitSource standardTickUnits) {
        ...
        this.gridlinePaint = DEFAULT_GRIDLINE_PAINT;
     ...
     * @param edge  the edge.
     */
    public void drawAxisLine(Graphics2D g2, double cursor,
    ...
    protected AxisState drawTickMarksAndLabels(Graphics2D g2,
        ...
        while (iterator.hasNext()) {
            ...
            if (isTickLabelsVisible()) {
				...
				float[] anchorPoint = calculateAnchorPoint(tick, cursor,
						dataArea, edge);
				//Added by Brian Cabana 8/20/07 
				if (useRotatedTickLabels && tick instanceof NumberBlockTick) {
					CategoryLabelPosition position = this.categoryLabelPositions
							.getLabelPosition(edge);
					TextBlock block = ((NumberBlockTick) tick).getLabel();
					block.draw(g2, anchorPoint[0], anchorPoint[1], position
							.getLabelAnchor(), anchorPoint[0], anchorPoint[1],
							position.getAngle());

				} else {
					TextUtilities.drawRotatedString(tick.getText(), g2,
							anchorPoint[0], anchorPoint[1], tick
									.getTextAnchor(), tick.getAngle(), tick
									.getRotationAnchor());
				}
				// need to draw the tick relative to anchor point if using auto
				// tick spacing
				if (isTickMarksVisible() && isAutoTickSpacing()) {
					Line2D mark = null;
					g2.setStroke(getTickMarkStroke());
					g2.setPaint(getTickMarkPaint());
					if (edge == RectangleEdge.LEFT) {
						mark = new Line2D.Double(cursor - ol, anchorPoint[1],
								cursor + il, anchorPoint[1]);
					} else if (edge == RectangleEdge.RIGHT) {
						mark = new Line2D.Double(cursor + ol, anchorPoint[1],
								cursor - il, anchorPoint[1]);
					} else if (edge == RectangleEdge.TOP) {
						mark = new Line2D.Double(anchorPoint[0], cursor - ol,
								anchorPoint[0], cursor + il);
					} else if (edge == RectangleEdge.BOTTOM) {
						mark = new Line2D.Double(anchorPoint[0], cursor + ol,
								anchorPoint[0], cursor - il);
					}
        ...
        // update the tick spacing for auto ticks
        if (isAutoTickSpacing() && ticks.size() > 0) {
            double max = ticks.size() > 1 ? ((ValueTick)ticks.get(ticks.size() -1)).getValue() : getUpperBound();
            double min = ((ValueTick)ticks.get(0)).getValue();
            if (ticks.size() <= 1)
            	setTickSpacing(max - min, false);
            else
            	setTickSpacing((max - min)/ (ticks.size() - 1), false);
        }
        
        // If we need to draw the tick marks for manual tick spacing
        if (isTickMarksVisible() && !isAutoTickSpacing()) {
            double endx = getUpperBound();
            double startx = ticks.size() == 0 ? getLowerBound() : ((ValueTick) ticks.get(0)).getValue();
            double width = getTickSpacing();
            if (width <= 0)
                width = endx - startx;
            for (double x = startx; x < endx; x += width) {
    ...
    public AxisSpace reserveSpace(Graphics2D g2, Plot plot,
        ...
        if (isTickLabelsVisible()) {
            ...
            if (RectangleEdge.isTopOrBottom(edge)) {
                tickLabelHeight = findMaximumTickLabelHeight(ticks, g2,
                        plotArea, isVerticalTickLabels(), edge);
				tickLabelWidth = findMaximumTickLabelWidth(ticks, g2, plotArea,
						isVerticalTickLabels(), RectangleEdge.RIGHT);
            }
            else if (RectangleEdge.isLeftOrRight(edge)) {
                tickLabelWidth = findMaximumTickLabelWidth(ticks, g2, plotArea,
                        isVerticalTickLabels(), edge);
 				tickLabelHeight = findMaximumTickLabelHeight(ticks, g2,
						plotArea, isVerticalTickLabels(), RectangleEdge.TOP);
           }
        }
        ...
        if (RectangleEdge.isTopOrBottom(edge)) {
            ...
            space.setRight(Math.max(space.getRight(), tickLabelWidth / 2));
        } else if (RectangleEdge.isLeftOrRight(edge)) {
            labelWidth = labelEnclosure.getWidth();
            space.add(labelWidth + tickLabelWidth, edge);
            space.setTop(Math.max(space.getTop(), tickLabelHeight / 2));
       }
    ...
    protected double findMaximumTickLabelHeight(List ticks,
		...
		if (useRotatedTickLabels) {
			Iterator iterator = ticks.iterator();
			CategoryLabelPosition position = getCategoryLabelPositions()
					.getLabelPosition(edge);
			while (iterator.hasNext()) {
				Tick tick = (Tick) iterator.next();
				if (tick instanceof NumberBlockTick) {
					TextBlock block = ((NumberBlockTick) tick).getLabel();
					maxHeight = Math.max(maxHeight, calculateTextBlockHeight(
							block, position, g2));
				}
			}
		} else {
			if (vertical) {
				...
				while (iterator.hasNext()) {
					Tick tick = (Tick) iterator.next();
					Rectangle2D labelBounds = TextUtilities.getTextBounds(tick
							.getText(), g2, fm);
				...
				LineMetrics metrics = font.getLineMetrics("ABCxyz", g2
						.getFontRenderContext());
				maxHeight = metrics.getHeight() + insets.getTop()
						+ insets.getBottom();
			}
		}
		return maxHeight;
    ...
    protected double findMaximumTickLabelWidth(List ticks, Graphics2D g2,
            Rectangle2D drawArea, boolean vertical, RectangleEdge edge) {
		...
		if (useRotatedTickLabels) {
			...
			while (iterator.hasNext()) {
				...
				if (tick instanceof NumberBlockTick) {
					TextBlock block = ((NumberBlockTick) tick).getLabel();
					maxWidth = Math.max(maxWidth, calculateTextBlockWidth(
							block, position, g2));
				}
			}
		} else {
			if (!vertical) {
				...
				LineMetrics metrics = font.getLineMetrics("ABCxyz", g2
						.getFontRenderContext());
				maxWidth = metrics.getHeight() + insets.getTop()
						+ insets.getBottom();
			}
		}
		return maxWidth;
    ...
    public void setLowerMargin(double margin) {
    	setLowerMargin(margin, true);
    }
    
    /**
    * Sets the lower margin for the axis (as a percentage of the axis range)
     * and sends an {@link AxisChangeEvent} to all registered listeners if notify
     * is <code>true</code>. This margin is added only when the axis range is 
     * auto-calculated - if you set the axis range manually, the margin is ignored.
     * 
     * @param margin
     ...
     *
     * @see #getLowerMargin()
     * @see #setUpperMargin(double, boolean)
     */
    public void setLowerMargin(double margin, boolean notify) {
     ...
     *            the margin percentage (for example, 0.05 is five percent).
     * @param notify
     * 			  a boolean to indicate if listeners should be notified
     * @see #getLowerMargin()
     * @see #setLowerMargin(double)
     */
   public void setUpperMargin(double margin, boolean notify) {
    ...
    public void setAutoOrigin(boolean b) {
        if (b != autoOrigin)
        {
            autoOrigin = b;
            notifyListeners(new AxisChangeEvent(this));            
	...
	public boolean isUseRotatedTickLabels() {
		return useRotatedTickLabels;
	}

	public void setUseRotatedTickLabels(boolean useRotatedTickLabels) {
		this.useRotatedTickLabels = useRotatedTickLabels;
        notifyListeners(new AxisChangeEvent(this));
	}
