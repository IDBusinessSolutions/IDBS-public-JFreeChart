 ...
 * 8/24/2007 bcabana Fixed bug QM-6655 where LogAxis width computation was off and QM-6657 Better label text measurement for rotated labels
 * 8/31/2007 bcabana QM-6672 and QM-6671 Log axis problems
 * 9/10/2007 bcabana QM-6746 Labels clipped because wrong font
 * 2/26/2008 bcabana QM-7481 and QM-7523, QM-7677 Multi-y-axis fixes
 * 7/8/2008 bcabana QM-8094: Additional model visualization / interaction gadgets QM-8094 Implemented a Dial Gauge and Thermometer
 */
...
public class LogarithmicAxis extends NumberAxis {
	...
	public AxisSpace reserveSpace(Graphics2D g2, Plot plot,
        ...
        // To Fix a bug where the the label is clipped due to resting right on
		// end of line. If the upper is the log ceiling then add extra space
		double upper = getUpperBound();
        if (upper == computeLogCeil(upper) && isTickLabelsVisible()) {
			double labelMargin = 0;
			g2.setFont(getTickLabelFont());
			AxisState state = new AxisState();
			List ticks = refreshTicks(g2, state, plotArea, edge);
			if (RectangleEdge.isTopOrBottom(edge)) {
				labelMargin = findMaximumTickLabelWidth(ticks, g2, plotArea,
						isVerticalTickLabels(), edge);
				space.setRight(Math.max(space.getRight(), labelMargin / 2));
			} else if (RectangleEdge.isLeftOrRight(edge)) {
				labelMargin = findMaximumTickLabelHeight(ticks, g2, plotArea,
						isVerticalTickLabels(), edge);
				space.setTop(Math.max(space.getTop(), labelMargin / 2));
			}
		}
		return space;
	}
	...
	public List refreshTicks(Graphics2D g2, AxisState state,
		...
		for (int i = iBegCount; i <= iEndCount; i++) {
			...
			for (int j = 0; j < jEndCount; j++) {
				...
				if (tickVal >= lowerBoundVal - SMALL_LOG_VALUE) {
					//tick value not below lowest data value
					CategoryLabelPosition position = this
							.getCategoryLabelPositions().getLabelPosition(edge);
					float l = 0.0f;
					if (position.getWidthType() == CategoryLabelWidthType.CATEGORY) {
						if (RectangleEdge.isLeftOrRight(edge))
							l = (float) dataArea.getHeight() / (iEndCount - iBegCount + 1);
						else
							l = (float) dataArea.getWidth() / (iEndCount - iBegCount + 1);
					} else if (RectangleEdge.isLeftOrRight(edge)) {
						l = (float) dataArea.getWidth();
					} else {
						l = (float) dataArea.getHeight();
					}
					float r = position.getWidthRatio();
					TextBlock label = TextUtilities.createTextBlock(tickLabel,
							getTickLabelFont(), getTickLabelPaint(), r * l, 1,
							new G2TextMeasurer(g2));
					//create tick object and add to list:
					ticks.add(new NumberBlockTick(new Double(tickVal), tickLabel, label,
							position.getLabelAnchor(), position.getRotationAnchor(), position
									.getAngle()));
				}
			}
		}
