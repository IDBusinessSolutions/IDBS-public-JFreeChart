 ...
 * 9/19/2007 bcabana QM-6781 category axis supports grid and tick spacing
 */
...
public class CategoryAxis extends Axis implements Cloneable, Serializable {
    ...
    public double getCategoryStart(int category, int categoryCount,
        ...
        result += category * (categorySize + categoryGapWidth) + categoryGapWidth / 2;
    ...
    public double calculateCategoryGapSize(int categoryCount,
                                              Rectangle2D area,
                                              RectangleEdge edge) {
	//Quantrix change - visibility changed to public to allow access
        double result = 0.0;
        double available = 0.0;
    ...
    public AxisState draw(Graphics2D g2, double cursor, Rectangle2D plotArea,
        ...
        //Quantix need more info for axis rendering
        registerAxisRenderingInfo(plotState, dataArea, edge, cursor, state);
        return state;
    ...
    protected AxisState drawCategoryLabels(Graphics2D g2,
        ...
        if (isTickLabelsVisible()) {
            ...
            if (isTickMarksVisible()) {
                double tickSpacing = Math.max(getTickSpacing(), 1);
                int columnCount = state.getTicks().size();
                double startx = getCategoryJava2DCoordinate(
                        CategoryAnchor.MIDDLE, 0, columnCount, dataArea, edge);
                double width = (calculateCategorySize(columnCount, dataArea,
                        edge) + calculateCategoryGapSize(columnCount, dataArea,
                        edge))
                        * tickSpacing;
                double endx = dataArea.getMaxX();
                for (double xx = startx; xx < endx; xx += width) {
                    Line2D mark = null;
                    g2.setStroke(getTickMarkStroke());
                    g2.setPaint(getTickMarkPaint());
                    if (edge == RectangleEdge.LEFT) {
                        mark = new Line2D.Double(cursor - ol, xx, cursor + il,
                                xx);
                    } else if (edge == RectangleEdge.RIGHT) {
                        mark = new Line2D.Double(cursor + ol, xx, cursor - il,
                                xx);
                    } else if (edge == RectangleEdge.TOP) {
                        mark = new Line2D.Double(xx, cursor - ol, xx, cursor
                                + il);
                    } else if (edge == RectangleEdge.BOTTOM) {
                        mark = new Line2D.Double(xx, cursor + ol, xx, cursor
                                - il);
    ...
    public List refreshTicks(Graphics2D g2,
        ...
        //Need to ensure that we have the correct font for measurement
       	g2.setFont(getTickLabelFont());
        ...
        if (categories != null) {
			...
			if (isAutoTickSpacing()) {
				double w;
				if (RectangleEdge.isLeftOrRight(edge))
					w = (double) dataArea.getHeight() / categories.size();
				else
					w = (double) dataArea.getWidth() / categories.size();
				double a = Math.max(1 - Math.cos(position.getAngle()), 0.6);
				spacing = (int) Math.ceil(20 / w / a);
				setTickSpacing(spacing, false);
			} else {
				spacing = Math.max((int) getTickSpacing(), 1);
			}
			if (isAutoGridSpacing())
				setGridSpacing(spacing, false);
            ...
            while (iterator.hasNext()) {
				...
				TextBlock label;
				if (categoryIndex % spacing == 0)
					label = createLabel(category, l * spacing * r, edge, g2);
				else
					label = createLabel("", l * spacing * r, edge, g2);
				if (edge == RectangleEdge.TOP || edge == RectangleEdge.BOTTOM) {
