 ...
 * 7/11/2007 bcabana work on charting
 ...
 * 9/5/2007 bcabana scatter chart x axis custom grid spacing shows infinity with incomplete data
 * 
 */
...
import org.jfree.chart.plot.AxisRenderingInfo;
...
public abstract class Axis implements Cloneable, Serializable {
    ...
    /** boolean indicating whether to auto-calculate the grid spacing */
    protected boolean autoGridSpacing = true;

    /** The setting which determines how frequently grid lines are displayed. */
    protected double gridSpacing;

    /** boolean indicating whether to auto-calculate the tick spacing */
    protected boolean autoTickSpacing = true;

    /**
     * The setting which determines how frequently axis tick marks are
     * displayed.
     */
    ...
    protected AxisState drawLabel(String label, Graphics2D g2,
        ...
        labelBounds = new Rectangle2D.Double(labelx - labelBounds.getWidth()/2, labely - labelBounds.getHeight()/2, labelBounds.getWidth(), labelBounds.getHeight());
    ...
    public void drawAxisLine(Graphics2D g2, double cursor,
            Rectangle2D dataArea, RectangleEdge edge) {
		//Quantrix - Changed visibility of method to public
    ...
    protected AxisRenderingInfo registerAxisRenderingInfo(
            PlotRenderingInfo plotState, Rectangle2D dataArea,
            RectangleEdge edge, double start, AxisState state) {
        Rectangle2D area;
        double end = state.getCursor();
        double min = Math.min(start, end);
        double size = Math.abs(end - start);

        if (edge == RectangleEdge.BOTTOM || edge == RectangleEdge.TOP)
            area = new Rectangle2D.Double(dataArea.getMinX(), min, dataArea
                    .getWidth(), size);
        else
            area = new Rectangle2D.Double(min, dataArea.getMinY(), size,
                    dataArea.getHeight());

        AxisRenderingInfo info = new AxisRenderingInfo(area);
        info.setLabelArea(state.getLabelBounds());
       	//Quantrix Change - The PolarPlot calls this method without a plotState so check
       	//for null (Brian Cabana 7/11/07
        if (plotState != null)
        	plotState.putAxisInfo(this, info);

        return info;
    }
 
    public void setGridSpacing(final double spacing) {
        setGridSpacing(spacing, true);
    }
    
    public void setGridSpacing(final double spacing, final boolean notify) {
    ...
    public void setAutoGridSpacing(boolean flag) {
        if (flag != autoGridSpacing) {
            autoGridSpacing = flag;
            notifyListeners(new AxisChangeEvent(this));
        }
    }

    public boolean isAutoGridSpacing() {
        return autoGridSpacing;
    }

    public void setTickSpacing(final double spacing) {
        setTickSpacing(spacing, true);
    }
    
    public void setTickSpacing(final double spacing, final boolean notify) {
        if (spacing != tickSpacing) {
            tickSpacing = spacing;
            if (notify)
                notifyListeners(new AxisChangeEvent(this));
        }
    ...
    public void setAutoTickSpacing(boolean flag) {
        if (flag != autoTickSpacing) {
            autoTickSpacing = flag;
            notifyListeners(new AxisChangeEvent(this));
        }
    }
