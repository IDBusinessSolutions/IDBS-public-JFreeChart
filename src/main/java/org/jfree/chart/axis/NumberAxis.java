   ...
   protected void selectHorizontalAutoTickUnit(Graphics2D g2,
    ...
    public List refreshTicks(Graphics2D g2,
        ...
        if (!useRotatedTickLabels) {
			...
			// Implementation of rotated labels by Brian Cabana 8/20/09
			...
			if (count <= ValueAxis.MAXIMUM_TICK_COUNT) {
				for (int i = 0; i < count; i++) {
					...
					if (formatter != null) {
						...
						tickLabel = getTickUnit().valueToString(
								currentTickValue);
					}
					...
					if (position.getWidthType() == CategoryLabelWidthType.CATEGORY) {
						if (RectangleEdge.isLeftOrRight(edge))
							l = (float) dataArea.getHeight() / count;
						else
							l = (float) dataArea.getWidth() / count;
					} else if (RectangleEdge.isLeftOrRight(edge)) {
					...
					Tick tick = new NumberBlockTick(
							new Double(currentTickValue), label, position
									.getLabelAnchor(), position
									.getRotationAnchor(), position.getAngle());
					result.add(tick);
				}
			}
		}
