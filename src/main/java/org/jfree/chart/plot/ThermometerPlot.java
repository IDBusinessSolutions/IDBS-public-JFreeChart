 ...
 * 3/27/2012 bcabana Worked on thermometer settings to get a better look
 * 3/28/2012 bcabana Added new properties to the Thermometer (border color) and Dial guage (minor ticks)
 */
    ...
    public void draw(Graphics2D g2, Rectangle2D area, Point2D anchor,
        ...
        if ((this.dataset != null) && (this.dataset.getValue() != null)) {
            ...
            if (this.subrangeIndicatorsVisible) {
                ...
                //Quantrix Edit (Brian Cabana)
                //Changed the warning markers to paint under the thermometer range
                double x1 = midX - getColumnRadius() + 1;
                double x2 = midX + getColumnRadius() - 1;
                switch (this.axisLocation) {
                case RIGHT:
                    x1 = midX + getColumnRadius();
                    x2 = x1 + 4;
                    break;
                case LEFT:
                    x1 = midX - getColumnRadius() - 4;
                    x2 = x1 + 4;
                    break;
                }
                ...
                if (range.contains(value)) {
                	...
                	double y2 = this.rangeAxis.valueToJava2D(subrangeInfo[NORMAL][RANGE_HIGH], dataArea, RectangleEdge.LEFT);
                	g2.fill(new Rectangle2D.Double(x1, y2, x2 - x1, y - y2));
                    }
                }
                ...
                if (range.contains(value)) {
                	...
                	double y2 = this.rangeAxis.valueToJava2D(subrangeInfo[WARNING][RANGE_HIGH], dataArea, RectangleEdge.LEFT);
                	g2.fill(new Rectangle2D.Double(x1, y2, x2 - x1, y - y2));                	
                    }
                }
                ...
                if (range.contains(value)) {
                    ...
                    double y = this.rangeAxis.valueToJava2D(value, dataArea,
                 	...
                 	double y2 = this.rangeAxis.valueToJava2D(subrangeInfo[CRITICAL][RANGE_HIGH], dataArea, RectangleEdge.LEFT);
                 	g2.fill(new Rectangle2D.Double(x1, y2, x2 - x1, y - y2));                	
                     }
                }
            }
